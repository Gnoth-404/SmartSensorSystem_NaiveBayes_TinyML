...
#define LED7_OFF					rp_DpinSetState( RP_LED7, RP_LOW  )
...
#define LED7_ON						rp_DpinSetState( RP_LED7, RP_HIGH )

void LEDx_INIT() {
    for(int i = 0; i <8; i++) {
        rp_DpinSetDirection(i+RP_LED0, RP_OUT);
        rp_DpinSetState(i+RP_LED0, RP_LOW);
    }
}
#define NUM_BUTTONS 4
#define NUM_LEDS 8
#define BUTTON_DEBOUNCE_DELAY_US 100000  // 100ms
...
void handleButtonPress(ButtonFunction button) {
    switch (button) {
        case BAYES_BUTTON:
            iic_set_run(RUN_BAYES, 1);
            LED5_ON;
            LED6_OFF;
            break;
        case BOX_BUTTON:
            iic_set_run(RUN_BOX, 1);
            break;
        case LOG_HUMAN_BUTTON:
            iic_set_log(100, 1, 4, SAVE_FEAT_HUMAN);
            break;
        case LOG_NONHUMAN_BUTTON:
            iic_set_log(100, 2, 4, SAVE_FEAT_NONHUMAN);
            break;
        // Add other button handlers as needed
    }
}

// Thread No. 3 for detecting Button state!
/*
*	@brief		Thread waits for signal from Button and executing command
*	@name		Button Thread
*/
void ButtonThread(void *threadid)
{
    ...
    // Set the direction of the button pins to input
    ...
    while(1)
    {
        for (int i=0; i<NUM_BUTTONS; i++) {
			...
			if (state == RP_LOW){
				handleButtonPress((ButtonFunction)i);
			}
        }
        usleep(BUTTON_DEBOUNCE_DELAY_US);  // Delay to avoid debouncing issues and reduce CPU usage //100ms
    }
	rp_Release(); // release Resources
    pthread_exit(NULL);
}
...
static feat_t				feat;
static send_feat_t			send_feat_data;
...
static adc_t				adc;
static fft_t				fft;
static uint16_t				cnt_log_adc;			// max: 10.000 measurements --> 1000 seconds ( 10 measurements / file )(logging adc)
static uint16_t				cnt_log_fft;			// max: 10.000 measurements --> 1000 seconds ( 10 measurements / file )(logging fft)
static uint16_t				cnt_log_feat;			// max: 10.000 measurements --> 1000 seconds ( 10 measurements / file ) (features logging) 
static uint8_t				sensor_number	=     0;


static object_t				currentObject;
...
rp_dpin_t					led_4 = RP_LED4;
rp_dpin_t					led_5 = RP_LED5;
...
int start_adc( adc_t *a, int16_t *data, struct iic_s *i )
{
	...
	// calculate TOF in µs , using 16 bit datas / 2 8 bits
...
void parseValues(feat_t *feat, float values[]){
    values[0]= feat->F1;
    values[1]= feat->F2;
    values[2]= feat->F3;
    values[3]= feat->F6;
...
void MeasureThread( void *threadid )
{
	...
	// pData is a pointer to the data array of the Header struct
	...
	while( 1 )
	{
		...
		// if distance is valid, and not in run ADC mode, run save data
		if( retval && ( run != RUN_ADC ) )
		{
			//logging adc before calculting feature, fft
			if( cnt_log_adc > 0 )
			{
				cnt_log_adc--;
				Header.DataLength = ADC_BUFFER_SIZE * 2;

				save_data_binary(&Header, (Header.HeaderLength / 2 + ADC_BUFFER_SIZE), SAVE_ADC );
									// length in byte --> 32.768 * 2
				if( cnt_log_adc == 0 ){
					LED3_OFF;
				}
			}
			...
			if( cnt_log_feat > 0)
			{
				...
				//Header.DataLength	= (10) * sizeof(uint16_t);
				switch (currentObject){
					case NONE:
						break;
					case HUMAN:
						//printf("Button 3 Pressed-SAVE_FEAT_HUMAN\n");
						//printf("Prediction: %d\n", model_f10_predict((const float *)&feat, 10));
						save_data_feature(&feat, 10, SAVE_FEAT_HUMAN);
						break;
					case NON_HUMAN:
						save_data_feature(&feat, 10, SAVE_FEAT_NONHUMAN);
						break;
					case WALL:
						break;
					default:
			...
			if( cnt_log_fft > 0 )
			{
				...
				save_data_binary( &Header, (Header.HeaderLength / 2 + (Header.DataLength / 2) ), SAVE_FFT);
				if( cnt_log_fft == 0 ){
					LED4_OFF;
				}
			}
		...
		if( send_fft ){
			send_fft = false; // set to false
			// send demo data
			Header.Class		= (float)(object_class);
			Header.Distance		= (float)(distance);
			Header.MeasDelay	= (float)adc.start_delay_us;
			// cpy pData to Header.p_data
			memcpy( (uint16_t *)Header.p_data, pData, ADC_BUFFER_SIZE * sizeof(uint16_t));
			send_binary( (uint16_t *)&Header, (uint16_t)(Header.HeaderLength + Header.DataLength) ); //send all of things
		}
		else if (send_feat){
			send_feat = false;
			parseValues(&feat, values);
			send_feat_data.feat = feat;
			send_feat_data.highest_idx=model_f10_predict(values, 4);
			send_feat_data.distance = (float)(distance);
			printf("Highest index: %d\n", send_feat_data.highest_idx);
				// class index =0 -> human
				// class index =1 -> non human
			//send feature data
			send_binary((uint16_t *)&send_feat_data, sizeof(send_feat_data));
...
//max: 10.000 measurements --> 1000 seconds ( 10 measurements / file ) (features logging) 
//saving directory
//logging adc feature fft
void iic_set_log( int16_t cnt_log_in, uint8_t ObjectType, uint8_t run_mode, const char *directory)
{
	currentObject = ObjectType;
	if( cnt_log_in < 0 )
	{
		//remove the filename that have been created
		init_data(directory);
	}
	else if( cnt_log_in <= 10000 )
	{
			cnt_log_adc = ( run_mode & 0x01 )?( cnt_log_in ): (0);
			cnt_log_fft = ( run_mode & 0x02 )?( cnt_log_in ): (0);
			cnt_log_feat = (run_mode & 0x04 )?( cnt_log_in): (0);
			( cnt_log_feat > 0 )?(LED5_ON):(LED5_OFF);

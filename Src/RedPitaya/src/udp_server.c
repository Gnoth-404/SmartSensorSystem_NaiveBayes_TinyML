...
void send_binary( uint16_t *data_in, size_t count )
{
	//uint16_t Sent;
	if( udp.socket >= 0 ){
		sendto( udp.socket, (uint16_t *)data_in, count, MSG_DONTWAIT,  (struct sockaddr *) &udp.clientaddr, udp.length);
	}
}
	...
	if( msg_rx[0] == '-' ){
		...
		switch( udp.command )
		{
			...
			case 'i':
				...
				break;
			case 'l':
				iic_set_log( (int16_t)udp.parameter[0], (uint8_t)udp.parameter[1], 1, SAVE_DATA_BINARY );
				break;
			case 'n':
			...
			case 'w':
				...
				break;
			case 'b':
				// start/stop bayes classifier
				iic_set_run( RUN_BAYES, (udp.parameter[0] == 1) );
				break;
			default:
			break;
